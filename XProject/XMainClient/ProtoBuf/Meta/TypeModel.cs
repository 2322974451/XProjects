...
namespace ProtoBuf.Meta
{
    public abstract class TypeModel
    {
            ...
            else
            {
                ...
                switch (typeCode)
                {
                    ...
                    default:
                        ...
                        if (!(value is IEnumerable enumerable4))
                            return false;
                        if (isInsideList)
                            throw TypeModel.CreateNestedListsNotSupported();
                        foreach (object obj in enumerable4)
                        {
                            if (obj == null)
        ...
        public void Serialize(Stream dest, object value) => this.Serialize(dest, value, (SerializationContext)null);
          ...
          SerializationContext context)
        {
            ...
            try
            {
                ...
                else if (!this.TryDeserializeAuxiliaryType(protoReader, DataFormat.Default, 1, type, ref value, true, false, true, false) && (uint)num > 0U)
                    TypeModel.ThrowUnexpectedType(type);
        ...
        public object Deserialize(Stream source, object value, Type type) => this.Deserialize(source, value, type, (SerializationContext)null);
        ...
        public object Deserialize(Stream source, object value, Type type, int length) => this.Deserialize(source, value, type, length, (SerializationContext)null);
          ...
          out bool isList)
        {
            ...
            {
                bool flag = type.IsInterface && type == model.MapType(typeof(IEnumerable<>)).MakeGenericType(types);
        ...
        private static bool CheckDictionaryAccessors(TypeModel model, Type pair, Type value) => pair.IsGenericType && pair.GetGenericTypeDefinition() == model.MapType(typeof(KeyValuePair<,>)) && pair.GetGenericArguments()[1] == value;
          ...
          ref object value)
        {
            ...
            object[] parameters = isList ? (object[])null : ProtoDecoratorBase.s_argsRead;
        ...
        internal bool TryDeserializeAuxiliaryType(
          ...
          bool insideList)
        {
            ProtoTypeCode code = type != null ? Helpers.GetTypeCode(type) : throw new ArgumentNullException(nameof(type));
            int modelKey;
        ...
        protected internal static void ThrowUnexpectedType(Type type)
        {
            ...
            Type type1 = type != null ? type.BaseType : throw new InvalidOperationException("Type is not expected, and no contract can be inferred: " + str);
            if (type1 != null && type1.IsGenericType && type1.GetGenericTypeDefinition().Name == "GeneratedMessage`2")
        ...
        internal static Exception CreateNestedListsNotSupported() => (Exception)new NotSupportedException("Nested or jagged lists and arrays are not supported");
        ...
        {
            Type type1 = type != null ? Helpers.GetUnderlyingType(type) : throw new ArgumentNullException(nameof(type));
            if (type1 != null)
                type = type1;
        ...
        {
            get => this.forwardsOnly;
            set => this.forwardsOnly = value;
        }

        internal virtual Type GetType(string fullName, Assembly context) => TypeModel.ResolveKnownType(fullName, this, context);
        ...
        internal static Type ResolveKnownType(string name, TypeModel model, Assembly assembly)
        {
            ...
            try
            {
                ...
                Type type = assembly == null ? (Type)null : assembly.GetType(name1);
                if (type != null)
                    return type;
            }
            catch
        ...
        private sealed class DeserializeItemsIterator<T> :
          TypeModel.DeserializeItemsIterator,
          IEnumerator<T>,
          IEnumerator,
          IDisposable,
          IEnumerable<T>,
          IEnumerable
        {
            ...
            public DeserializeItemsIterator(
              ...
              int expectedField,
              SerializationContext context)
              : base(model, source, model.MapType(typeof(T)), style, expectedField, (Serializer.TypeResolver)null, context)
            {
            }
        }

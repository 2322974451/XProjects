...
namespace ProtoBuf
{
    public sealed class ProtoReader : IDisposable
    {
        ...
        private int TryReadUInt64VariantWithoutMoving(out ulong value)
        {
            ...
            value |= num18 << 63;
            if ((num18 & 18446744073709551614UL) > 0UL)
                throw ProtoReader.AddErrorData((Exception)new OverflowException(), this);
            return 10;
        }
        ...
        public void ThrowEnumException(Type type, int value) => throw ProtoReader.AddErrorData((Exception)new ProtoException("No " + (type == null ? "<null>" : type.FullName) + " enum is mapped to the wire-value " + value.ToString()), this);
        ...
        public unsafe float ReadSingle()
        {
            switch (this.wireType)
            {
                case WireType.Fixed64:
                    ...
                    return !Helpers.IsInfinity(num2) || Helpers.IsInfinity(num1) ? num2 : throw ProtoReader.AddErrorData((Exception)new OverflowException(), this);
                case WireType.Fixed32:
        ...
        public static byte[] AppendBytes(byte[] value, ProtoReader reader)
        {
            ...
            reader.wireType = WireType.None;
            if (count1 == 0)
                return value == null ? ProtoReader.EmptyBlob : value;
            int toIndex;
        ...
        internal void CheckFullyConsumed()
        {
            if (this.isFixedLength)
            {
                if ((uint)this.dataRemaining > 0U)
                    throw new ProtoException("Incorrect number of bytes consumed");
            }
            else if ((uint)this.available > 0U)
